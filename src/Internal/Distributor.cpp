...
namespace Internal
{
	...
	{
		for (const auto& [musicType, musicTrack] : Maps::general_prep_map) {
			logger::info(FMT_STRING("parser: [{}, {}]"), musicType, musicTrack);
			uint32_t musicTypeID = std::stoi(musicType.substr(0, musicType.find("~")));
			std::string_view musicTypeMod = musicType.substr(musicType.find("~"), musicType.length() - 1);
			logger::info(FMT_STRING("musicTypeID: id:{}, modname: "), musicTypeID, musicTypeMod);

			// uint32_t musicFormID = std::stoi(musicTrack);
			// // todo need to split by ~ and set to getformfrom mod for value after split
			// logger::info(FMT_STRING("musicFormID: {}"), musicFormID);
			// const auto musicForm = RE::TESForm::GetFormByID(musicFormID);
			// if (!musicForm) {
			// 	logger::info(FMT_STRING("musicForm was none"));
			// 	continue;
			// }
			// else {
			// 	logger::info(FMT_STRING("musicForm was filled"));
			// }
		}
	}
}
